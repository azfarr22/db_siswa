...
if (isset($_POST["update"])) {
    ...
    if (empty($photo_name)) {
        $sql = $pdo->prepare("UPDATE table_siswa SET nis=:nis, name=:name, gender=:gender, phone_number=:phone_number, address=:address WHERE id=:id");
        ...
        $sql = $pdo->prepare("SELECT photo FROM table_siswa WHERE id=:id");
        $sql->bindParam(':id', $id);
        $sql->execute();
        $data = $sql->fetch();
        ...
        $sql = $pdo->prepare("UPDATE table_siswa SET nis=:nis, name=:name, gender=:gender, phone_number=:phone_number, address=:address, photo=:photo WHERE id=:id");
        $sql->bindParam(':nis', $nis);
        $sql->bindParam(':name', $name);
        $sql->bindParam('gender', $gender);
        $sql->bindParam(':phone_number', $phone_number);
